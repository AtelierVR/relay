...
namespace Relay.Utils
{
    public class Config
    {
        ...
        public static bool HasEnv(string key) => Environment.GetEnvironmentVariable($"nox_{key}".ToUpper()) != null;

        public void Set<T>(string propertyName, T? value) => _jsonObject[propertyName] = value == null ? null : JToken.FromObject(value);
        ...
        public string GetUseAddress() => Get<string>("use_address") ?? GetFromEnv<string>("use_address") ?? Constants.UseAddress;
    }
}