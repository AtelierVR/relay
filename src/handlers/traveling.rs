...
pub async fn handle(state: &AppState, client_id: u16, uid: u16, payload: Bytes) -> Bytes {
    ...
    let inst_arc = match state.instances.get(iid) {
        Some(a) => a,
        None => return make_response(uid, iid, TravelingResults::Unknown, Some("Instance not found.")),
    };
    ...
    let self_player_id = {
        ...
        match inst.get_players().iter().find(|p| p.client_id == client_id) {
            Some(p) => p.id,
            None => return make_response(uid, iid, TravelingResults::Unknown, Some("You are not in the instance.")),
        }
    };
...
fn do_travel(state: &AppState, uid: u16, iid: u8, player_id: u16, inst_arc: &crate::instance::ArcInstance) -> Bytes {
    // Player must not be None.
    let status = {
        let inst = inst_arc.read();
        inst.get_player(player_id).map(|p| p.status).unwrap_or(PlayerStatus::None)
    };
    if status == PlayerStatus::None {
        return make_response(uid, iid, TravelingResults::Unknown, Some("Player is not ready to travel."));
    }
    ...
    let (master_id, address, version) = {
        ...
        if w.master_id == 0 && w.address.is_empty() {
            return make_response(uid, iid, TravelingResults::Unknown, Some("World is not set for the instance."));
        }
    ...
    info!("Traveling: player {player_id} traveling in instance {iid}");
...
fn do_ready(state: &AppState, uid: u16, iid: u8, player_id: u16, inst_arc: &crate::instance::ArcInstance) -> Bytes {
    let status = {
        let inst = inst_arc.read();
        inst.get_player(player_id).map(|p| p.status).unwrap_or(PlayerStatus::None)
    };
    ...
    if status != PlayerStatus::Traveling {
        return make_response(uid, iid, TravelingResults::Unknown, Some("Player is not traveling."));
    }
    ...
    info!("Traveling: player {player_id} is now Ready in instance {iid}");
